...
| `open-agent` | CE extension: open the Twin Agent inside the iframe |
...
- Connected Enterprise injects `ce-host-inventory-bridge.js` through its React
  ...
  existing host constellation and routes the host's Ctrl/Cmd+K shortcut to the
  Twin Agent without modifying the vendored hashed assets.
