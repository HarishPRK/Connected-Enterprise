...
7. The gateway consumes the GET response directly. It verifies the compact KMS-signed integrity claim, checks the Thing/gateway/generation/profile binding and expiry, requires `integrity.gatewayMetadataSha256` to match the canonical allowlisted `gateway` object, hashes the canonical inline `configuration` JSON, and compares it with both `integrity.profileSha256` and `assignment.profileChecksum`. The response contains the complete immutable profile document plus an allowlisted gateway object; it contains no S3 URL, object key, tenant record, certificate principal, or DynamoDB state. The gateway then applies transactionally and rolls back on failed health checks. Named Shadow `configuration` or an IoT Job may optionally notify the gateway that a later generation exists; configuration retrieval itself is HTTPS and uses no MQTT topic.
8. The gateway reports `APPLYING`, `HEALTH_CHECK`, and finally `APPLIED_HEALTHY` by SigV4-signing `POST /device/v1/things/{thingName}/certificates/{certificateId}/status` with the same short-lived credentials. `APPLIED_HEALTHY` contains the authoritative generation, `profileVersionId`, and lowercase SHA-256 `profileChecksum`. Only then does the operation become successful.
