...
7. The gateway consumes the GET response, verifies the signed descriptor and binding, downloads the exact immutable S3 object through the short-lived URL, applies transactionally, and rolls back on failed health checks. Named Shadow `configuration` or an IoT Job may optionally notify the gateway that a later generation exists; configuration retrieval itself is HTTPS and uses no MQTT topic.
