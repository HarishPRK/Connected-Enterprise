...
export function publicOperation(item: Record<string, unknown>): Record<string, unknown> {
  ...
  return {
    ...
    ...(item.resetForOnboarding === true ? { resetForOnboarding: true } : {}),
    ...(item.registrationReleased === true ? { registrationReleased: true } : {}),
    ...(item.resetError ? { resetError: item.resetError } : {}),
