...
import { appCategories, appPolicies } from '../data/applicationRoutingMock';
...
const POLICY_STORAGE_KEY = 'qsr.application-routing.policies.v1';

function loadPolicies(): AppPolicy[] {
  if (typeof window === 'undefined') return appPolicies;
  try {
    const saved = window.localStorage.getItem(POLICY_STORAGE_KEY);
    if (!saved) return appPolicies;
    const parsed = JSON.parse(saved);
    return Array.isArray(parsed) ? parsed as AppPolicy[] : appPolicies;
  } catch {
    return appPolicies;
  }
}
...
export function ApplicationAwareRoutingPage({ branchId }: { branchId: string }) {
  ...
  const [policies, setPolicies] = useState<AppPolicy[]>(loadPolicies);
  const { push } = useToast();
  ...
  useEffect(() => {
    try {
      ...
      // Storage is best-effort; private browser contexts can deny access.
    }
  }, [policies]);
  ...
  const list = useMemo(
    ...
    [policies, query, catFilter],
  );

  const totals = {
    apps: policies.length,
    enabled: policies.filter((p) => p.enabled).length,
    realtime: policies.filter((p) => p.slaClass === 'realtime').length,
    flowsPerMin: policies.reduce((s, p) => s + p.hitsPerMin, 0),
    throughput: policies.reduce((s, p) => s + p.throughputMbps, 0),
  };

  const topApps = [...policies]
  ...
  const togglePolicy = (policy: AppPolicy) => {
    setPolicies((current) => current.map((item) => (
      item.id === policy.id ? { ...item, enabled: !item.enabled } : item
    )));
    push({
      kind: 'info',
      title: `${policy.app} ${policy.enabled ? 'disabled' : 'enabled'}`,
      detail: 'Policy state saved in this browser.',
    });
  };

  const savePolicy = () => {
    if (!editing) return;
    const app = editing.app.trim();
    const match = editing.match.trim();
    if (!app || !match) {
      push({ kind: 'error', title: 'Policy needs an application and match rule' });
      return;
    }
    const isNew = !editing.id;
    const saved: AppPolicy = {
      ...editing,
      id: editing.id || `policy-${Date.now()}`,
      app,
      match,
    };
    setPolicies((current) => (
    ...
    push({
      ...
      detail: 'Saved in this browser. Publish live path changes from the steering patchboard.',
    });
    setEditing(null);
  };
  ...
  return (
    <>
      ...
      {/* KPI strip */}
      <div className="kpi-strip routing-kpi-strip">
      ...
      <div className="grid">
        ...
        <div className="col-12">
          <Card
            ...
            sub={`${list.length} shown · select a row to edit`}
            right={
              <div className="routing-policy-search" style={{ position: 'relative', display: 'inline-flex', alignItems: 'center' }}>
                <Search size={14} style={{ position: 'absolute', left: 10, color: 'var(--text-muted)' }} />
                <input
                  aria-label="Search routing policies"
                    ...
                    <tr key={p.id} style={{ cursor: 'pointer' }} onClick={() => setEditing(p)}>
                      ...
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <button
                            type="button"
                            aria-label={`Edit routing policy for ${p.app}`}
                            onClick={(event) => {
                      ...
                      <td onClick={(e) => e.stopPropagation()}>
                        <button
                          className="icon-btn"
                          style={{ width: 40, height: 40, color: p.enabled ? 'var(--ok)' : 'var(--text-muted)' }}
                          title={p.enabled ? 'Disable' : 'Enable'}
                          aria-label={`${p.enabled ? 'Disable' : 'Enable'} ${p.app}`}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <FormRow label="Application name">
              <input
                ...
                onChange={(event) => setEditing({ ...editing, app: event.target.value })}
              />
            </FormRow>
            <FormRow label="Match rule">
              <input
                value={editing.match}
                placeholder="e.g. *.salesforce.com or DSCP EF"
                onChange={(event) => setEditing({ ...editing, match: event.target.value })}
              />
            </FormRow>
            <FormRow label="Category">
              <select
                value={editing.category}
                onChange={(event) => setEditing({ ...editing, category: event.target.value as AppCategoryId })}
              >
            ...
            <FormRow label="SLA class">
              <select
                value={editing.slaClass}
                onChange={(event) => setEditing({ ...editing, slaClass: event.target.value as AppPolicy['slaClass'] })}
              >
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <FormRow label="Preferred path">
                <select
                  value={editing.preferredPath}
                  onChange={(event) => setEditing({ ...editing, preferredPath: event.target.value as AppPolicy['preferredPath'] })}
                >
              ...
              <FormRow label="Backup path">
                <select
                  value={editing.backupPath}
                  onChange={(event) => setEditing({ ...editing, backupPath: event.target.value as AppPolicy['backupPath'] })}
                >
