...
import { BRANCH_TO_IPSEC_SOURCE, appCategories } from '../../data/applicationRoutingMock';
...
export function AppSteeringPatchboard({ branchId }: { branchId: string }) {
  ...
  useEffect(() => {
    ...
    const tick = (frameMs: number) => {
      ...
      // The topology remains readable while idle, so 30fps is ample for cable
      // motion and materially lowers SVG attribute churn on integrated GPUs.
      if (!visible || document.hidden || frameMs - lastFrame < 1000 / 30) return;
      lastFrame = frameMs;
  ...
  return (
    <div className="app-steering-patchboard" style={{ position: 'relative', maxWidth: W, margin: '0 auto' }}>
      <svg
        ref={svgRef}
        viewBox={`0 0 ${W} ${H}`}
